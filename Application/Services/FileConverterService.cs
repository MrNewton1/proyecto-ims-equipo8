using System;
using System.IO;
using System.Drawing;

namespace Application
{
    /// <summary>
    /// Servicio para convertir archivos de imagen en cadenas Base64 y redimensionarlos.
    /// </summary>
    public static class FileConverterService
    {
        /// <summary>
        /// Cadena Base64 placeholder para usar cuando no se proporciona ninguna imagen.
        /// </summary>
        public static string PlaceHolder = "data:image;base64,";

        /// <summary>
        /// Convierte un flujo de archivo de imagen en una cadena Base64, redimensionando la imagen al ancho especificado.
        /// </summary>
        /// <param name="file">Flujo de datos de la imagen original.</param>
        /// <param name="w">Ancho en píxeles al cual redimensionar la imagen (por defecto 256).</param>
        /// <returns>Cadena de datos URI con la imagen codificada en Base64.</returns>
        /// <exception cref="FileLoadException">Si el flujo de datos está vacío.</exception>
        public static string ConvertToBase64(Stream file, int w = 256)
        {
            if (file.Length > 0)
            {
                var ms = new MemoryStream();
                file.CopyTo(ms);

                ms = ResizeImage(ms, w);

                var fileBytes = ms.ToArray();
                return "data:image;base64," + Convert.ToBase64String(fileBytes);
            }
            else
            {
                throw new FileLoadException();
            }
        }

        /// <summary>
        /// Redimensiona una imagen contenida en un MemoryStream a un nuevo ancho, manteniendo la proporción de aspecto.
        /// </summary>
        /// <param name="ms">MemoryStream que contiene la imagen original.</param>
        /// <param name="w">Nuevo ancho en píxeles.</param>
        /// <returns>MemoryStream con la imagen redimensionada.</returns>
        /// <exception cref="PlatformNotSupportedException">Si la plataforma no es Windows.</exception>
        public static MemoryStream ResizeImage(MemoryStream ms, int w)
        {
#if WINDOWS
            Image img = Image.FromStream(ms);
            int h = Convert.ToInt32(w * img.Height / img.Width);
            Bitmap imgN = new Bitmap(w, h);
            using (Graphics g = Graphics.FromImage(imgN))
            {
                g.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.HighQualityBicubic;
                g.DrawImage(img, 0, 0, w, h);
            }
            MemoryStream res = new MemoryStream();
            imgN.Save(res, img.RawFormat);
            res.Position = 0;
            return res;
#else
            throw new PlatformNotSupportedException("Image resizing is only supported on Windows platforms.");
#endif
        }
    }
}
